...
import com.company.julia.facade.PostFacade;
import com.company.julia.payload.response.MessageResponse;
import com.company.julia.service.PostService;
...
public class PostController {
    ...
    public ResponseEntity<List<PostDTO>> getAllPostsForUser(Principal principal) {
        List<PostDTO> postDTOList = postService.getAllPostsFromUser(principal)
                .stream()
                .map(postFacade::postToPostDTO)
                .collect(Collectors.toList());
        return new ResponseEntity<>(postDTOList, HttpStatus.OK);
